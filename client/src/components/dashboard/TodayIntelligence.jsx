...
const TodayIntelligence = () => {
  ...
  const consistencyConfig = {
    high:   { label: 'High',   color: 'text-success', bg: 'bg-emerald-500/10', border: 'border-emerald-500/20' },
    medium: { label: 'Medium', color: 'text-warning',   bg: 'bg-amber-500/10',   border: 'border-amber-500/20' },
    low:    { label: 'Low',    color: 'text-destructive',     bg: 'bg-red-500/10',     border: 'border-red-500/20' },
    none:   { label: '—',      color: 'text-muted-foreground',    bg: 'bg-secondary',        border: 'border-border' },
  };
  ...
  const kpis = [
    {
      ...
      icon: Clock,
      color: 'text-[hsl(var(--primary-accent))]',
      bg: 'bg-blue-500/10',
    ...
    },
    {
      ...
      icon: Target,
      color: focusScore >= 70 ? 'text-success' : focusScore >= 40 ? 'text-warning' : 'text-destructive',
  ...
  return (
    <motion.div
      ...
      className="border border-border bg-card rounded-md p-6 md:p-8 relative overflow-hidden"
    >
      {/* Background decoration */}
      <div className="absolute top-0 right-0 w-[200px] h-[200px] bg-blue-500/5 rounded-full hidden pointer-events-none" />

      {/* Header */}
      <div className="flex items-center justify-between mb-6 relative z-10">
        <div className="flex items-center gap-3">
          <div className="p-2 rounded-md bg-secondary border border-border">
            <Brain size={20} className="text-[hsl(var(--primary-accent))]" />
          </div>
          <div>
            <h2 className="text-lg font-bold text-foreground">Today's Intelligence</h2>
        ...
        {kpis.map((kpi, i) => {
          ...
          return (
            <motion.div
              ...
              className={`${kpi.bg} ${kpi.border} border rounded-md p-4 relative overflow-hidden group hover:scale-[1.02] transition-transform`}
            >
              {kpi.live && (
              ...
              <div className="text-xl font-semibold text-foreground mb-0.5">{kpi.value}</div>
              <div className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">{kpi.label}</div>
            </motion.div>
          );
        })}
      </div>
            ...
            {categoryData.map((cat, i) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 h-6 bg-secondary rounded-full overflow-hidden relative border border-border">
                    <motion.div
                      initial={{ width: 0 }}
                  ...
                  <div className="w-16 text-right shrink-0">
                    <span className="text-xs font-bold text-muted-foreground">{formatDuration(cat.duration)}</span>
                  </div>
                </motion.div>
              );
            })}
      ...
      {topCategory && (
        <div className="mt-5 flex items-center gap-3 px-4 py-3 bg-elevated border border-border rounded-md relative z-10">
          <Award size={18} className="text-[hsl(var(--primary-accent))] shrink-0" />
          <div className="flex-1 min-w-0">
            <span className="text-[11px] text-muted-foreground font-medium">Most focused on</span>
            <div className="text-sm font-bold text-foreground flex items-center gap-2">
              <div className="w-2 h-2 rounded-full" style={{ backgroundColor: topCategory.color }} />
              {topCategory.label}
              <span className="text-muted-foreground font-medium">— {formatDuration(categoryData[0].duration)}</span>
            </div>
          </div>
          <Sparkles size={14} className="text-[hsl(var(--primary-accent))]/50" />
        </div>
      )}
      ...
      {categoryData.length === 0 && !timerRunning && (
        <div className="text-center py-8 relative z-10">
          ...
          <p className="text-sm text-muted-foreground font-medium">No activity yet today</p>
          <p className="text-xs text-muted-foreground mt-1">Start a focus session to see your intelligence data</p>
        </div>
      )}
          ...
          <div className="flex gap-2 flex-wrap">
            {daily.peakHours
              ...
              .map((ph, i) => (
                <div
                  key={i}
                  className="px-3 py-1.5 bg-secondary border border-border rounded-md text-xs font-semibold text-muted-foreground"
                >
                  <span className="text-muted-foreground">{ph.hour.toString().padStart(2, '0')}:00</span>
                  <span className="text-muted-foreground ml-1.5">({ph.minutes}m)</span>
                </div>
              ))}
          </div>
        </div>
